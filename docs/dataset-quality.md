...
sanitized storage. Report version `dataset-quality-0.1.0` recomputes checks from
...
Configuration blocks are counted deterministically as non-empty command
regions separated by blank lines, Cisco-style `!` delimiters, or top-level
JunOS brace closures. Tokens use the report's versioned lexical approximation.
These are reproducible corpus metrics, not model-specific tokenizer counts.
The confirmed-anomaly value is supplied from a separately reviewed labeling
process and is never inferred from parser errors or detector output.
